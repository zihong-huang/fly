...
//内部
uint8_t BMP280_Read_Byte(uint8_t reg);								// 读取寄存器的值
void BMP280_Write_Byte(uint8_t reg, uint8_t data);		// 向寄存器写入值
...
u8  BMP280_GetStatus(u8 status_flag);									//获取状态
...
double BMP280_GetIIR_Temperature(void);								// 获取滤波后的温度值-℃
double BMP280_GetIIR_Pressure(void);									// 获取滤波后的大气压值-Pa
