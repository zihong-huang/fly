...
const u8 TX_ADDRESS_S[TX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //发送地址
const u8 RX_ADDRESS_S[RX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //接收地址					    
const u8 RX_ADDRESS_M[RX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //接收地址	
...
u8 NRF24L01_Read_Reg(u8 reg_addr)
{
	u8 reg_val;	    
 	CSN_L;                //使能SPI传输		
  	...
  	reg_val=SPI_ReadWriteByte(reg_addr);//读取寄存器内容
